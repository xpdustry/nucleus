...
public final class MessageBlockConfigurationFactory implements HistoryConfiguration.Factory<MessageBuild> {

    @Override
    public Optional<HistoryConfiguration> create(
            final MessageBuild building, final Type type, final @Nullable Object config) {
        if (config instanceof String message) {
            return Optional.of(HistoryConfiguration.Text.of(message, Text.Type.MESSAGE));
        } else {
